...
def analyze_with_deepseek(competitor, title, snippet):
    ...
    重点评估领域：寄件API、企业快递管理SaaS、智选运力、同城急送。
    ...
    请严格以 JSON 格式输出，必须且只能包含以下三个字段：
    1. "score": 影响程度打分 (整数，1-10分。1-4分为常规动态，5-7分为值得警惕，8-10分为严重威胁或重大商机)。
    2. "analysis": 简要分析竞对此举会如何影响我们的客户留存、API调用利润或SaaS市场份额（100字以内，一针见血）。
    3. "suggestion": 给业务团队的实操建议（一句话，例如降价应对、推出组合拳或跟进新功能）。
    """
    ...
    try:
        ...
        return result.get("score", 0), result.get("analysis", ""), result.get("suggestion", "")
    except Exception as e:
        print(f"⚠️ AI 分析遭遇波动: {e}")
        return 0, "AI分析暂时失败，稍后可手动重试", "无"
                ...
                for item in soup.find_all('h3'):
                    ...
                    if link and link.startswith("http") and "快递100" not in title:
                        ...
                        if cursor.fetchone() is None:
                            ...
                            # 唤醒 DeepSeek 分析
                            score, analysis, suggestion = analyze_with_deepseek(f"{comp}({kw})", title, snippet)
                            ...
                            cursor.execute('''
                                           ...
                                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                                           ''', (real_publish_time, comp, kw, title, snippet, link, score, analysis,
                                                 suggestion))
                            new_count += 1
