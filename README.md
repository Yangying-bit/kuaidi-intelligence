...
> 基于 Python + DeepSeek + GitHub Actions 的全自动化竞对情报监控系统，专为寄件 API 与 SaaS 业务场景打造。

## 项目亮点

本项目不仅是一个数据大盘，更是一个精打细算、完全托管在云端的自动化情报引擎：

* **AI 深度洞察**：接入 DeepSeek 模型（Flash版本），自动对长篇新闻进行“一句话摘要提取”、“业务威胁打分（1-10分）”与“高危预警”。
* **“零浪费”防重机制**：基于 SQLite 数据库的 `UNIQUE` 约束，抓取前秒级查重，完美避免重复调用大模型造成的 API 额度浪费。
* **黄金时段防漏策略**：GitHub Actions 定时任务采用“避峰运行（每 3 小时避开整点运行）” + “24 小时动态兜底抓取”策略，确保情报无遗漏且极度稳定。
